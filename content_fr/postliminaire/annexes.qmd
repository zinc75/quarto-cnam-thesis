...
## Option `validate` : vérification automatique depuis le script

Le script post-render peut interroger le webservice CINES automatiquement.
Dans `_quarto-fr.yml`, décommenter la ligne `validate` :
...
Remettre la ligne standard (sans `validate`) dans `_quarto-fr.yml` pour les rendus
quotidiens — l'appel au webservice n'est nécessaire qu'avant le dépôt final.
:::
